...
async def cmd_top(message: types.Message):
    ...
    try:
        # 2. Получаем Топ богачей (баланс)
        cursor.execute("SELECT custom_id, balance FROM users ORDER BY balance DESC LIMIT 10")
        top_bal = cursor.fetchall()
        
        # 3. Получаем Топ дня по профиту (из таблицы daily_stats)
