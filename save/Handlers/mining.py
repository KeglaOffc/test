...
async def trigger_random_event(user_id, current_pending):
    event_chance = random.randint(1, 100)
    # Шанс события 10%
    if event_chance > 10:
        return None, current_pending

    # Проверка защиты (из блока 4.2 Автоматизация)
    cursor.execute("SELECT antihack, overclock_shield FROM mining_farms WHERE user_id = ?", (user_id,))
    protection = cursor.fetchone()
    
    events = [
        # Тип, Название, Эффект (множитель или фикс), Описание
        ("bad", "⚡ Скачок напряжения", 0.7, "Износ оборудования сильно вырос!"),
        ("bad", "🔒 Хакерская атака", 0.8, "Хакеры украли 20% накопленного!"),
        ("bad", "👮 Проверка инспекции", -15000, "Штраф за шум: -15,000 coins"),
        ("good", "💎 Находка блока", 10000, "Вы нашли целый блок! +10,000 coins"),
        ("good", "🐳 Памп монеты", 2.0, "Курс взлетел! Доход за сбор удвоен!"),
        ("good", "🛠 Сервис", 1.0, "Ваше оборудование почищено и блестит!")
    ]
    
    event = random.choice(events)
    e_type, name, effect, desc = event

    # Логика защиты
    if name == "🔒 Хакерская атака" and protection[0] == 1:
        return "🛡 <b>Анти-хакер</b> отразил атаку! Вы ничего не потеряли.", current_pending
    ...
    if e_type == "bad":
        if isinstance(effect, float):
            ...
            new_pending = max(0, current_pending + effect)
        
        if name == "⚡ Скачок напряжения":
            cursor.execute("UPDATE mining_items SET wear = wear * 0.5 WHERE user_id = ?", (user_id,))
            
    elif e_type == "good":
        if name == "🛠 Сервис":
            cursor.execute("UPDATE mining_items SET wear = 100.0 WHERE user_id = ?", (user_id,))
        elif isinstance(effect, float):
            new_pending *= effect
        else:
            new_pending += effect
...
async def item_details(call: types.CallbackQuery):
    ...
    try:
        ...
        # Если текст остался прежним (нажал второй раз), просто убираем загрузку
        await call.answer()
    ...
    try:
        ...
        if "message is not modified" in str(e):
            # Если сообщение идентично, просто убираем анимацию загрузки на кнопке
            await call.answer()
        else:
...
async def buy_auto_module(call: types.CallbackQuery):
    ...
    cursor.execute(f"SELECT {module_key} FROM mining_farms WHERE user_id = ?", (user_id,))
    ...
    # Покупка
    cursor.execute(f"UPDATE users SET balance = balance - ? WHERE id = ?", (item['price'], user_id))
...
async def apply_random_event(user_id, current_pending):
    ...
    if chance > 15: # Событие происходит в 15% случаев
        return None, current_pending

    # Выбираем тип: 60% плохое, 40% хорошее
    is_bad = random.randint(1, 100) <= 60
    event_key = random.choice(list(NEG_EVENTS.keys() if is_bad else POS_EVENTS.keys()))
    event = NEG_EVENTS[event_key] if is_bad else POS_EVENTS[event_key]
    ...
    if event_key == "voltage":
        ...
        # Проверка защиты Анти-хакер (из п. 4.2)
        cursor.execute("SELECT antihack FROM mining_farms WHERE user_id = ?", (user_id,))
        if cursor.fetchone()[0] == 1:
            return "🛡 <b>Анти-хакер</b> отразил атаку! Вы ничего не потеряли.", current_pending
        new_pending *= 0.8
    elif event_key == "police":
        cursor.execute("UPDATE users SET balance = MAX(0, balance - 15000) WHERE id = ?", (user_id,))
    elif event_key == "rats":
        cursor.execute("UPDATE mining_items SET wear = MAX(0, wear - 15) WHERE user_id = ?", (user_id,))
...
@router.callback_query(F.data == "min:back")
