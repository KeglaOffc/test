...
def _build_catalog() -> Dict[str, Dict]:
    ...
    add(_event("ref_x2",          "🤝 Реферальный x2",         "Бонусы рефералки удваиваются.",              "ref_mult", 2.0, 24, ("ref",)))
    add(_event("ref_x3",          "🤝 Реферальный x3",         "Бонусы рефералки утраиваются.",              "ref_mult", 3.0, 12, ("ref",)))
    add(_event("ref_gift",        "🤝 Стартовый подарок",       "Новые рефералы получают +5 000 💎.",          "ref_gift", 5_000, 24, ("ref",)))
    add(_event("clan_tag_sale",   "🛡 Теги клана -50%",          "Создание тегов в клане -50%.",                "clan_tag_sale", 0.5, 12, ("clans",)))
    add(_event("clan_promote_fee","🛡 Повышение бесплатно",     "Промоут/демоут в клане без комиссий.",        "clan_promote_free", 1.0, 12, ("clans",)))
    ...
    add(_event("clan_season",     "🏁 Клановый сезон",            "Финиш: в конце недели топ-1 клан получает x2 к банку.","clan_top_mult", 2.0, 48, ("clans",)))
